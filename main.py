...
#
# experiment = ShockWaveExperiment(materials=[Kapton(),
#                                             MgO(is_stochastic=True),
#                                             Quartz(is_stochastic=True)])
#
# experiment.run_experiment(shock_pressure, std=0.05, initial_points=1000)
#
# experiment.plot()
#
...
# plt.savefig(f'uniform_up_kde_{datetime.datetime.now()}.png')
# sns.displot(data=df, x="Pressure (GPa)", kind="kde")
# plt.savefig(f'uniform_P_kde_{datetime.datetime.now()}.png')
# sns.displot(df, x="Particle Velocity (km/s)", y="Pressure (GPa)", kind='kde')
# plt.savefig(f'uniform_2D_kde_{datetime.datetime.now()}.png')
#
# a = 1
